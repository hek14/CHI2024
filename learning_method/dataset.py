...
def calc_orientation_graident(img, win_size=16, stride=8):
    ...
    return ori, np.stack((Gxy, Gxx - Gyy), -1)
...
class MatchDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        ori_img, ori_img_gt = calc_orientation_graident(img, win_size = 16, stride = 16)
        ori_patch,  = calc_orientation_graident(patch, win_size = 16, stride = 16)
        ...
        return img.astype(np.float32), patch.astype(np.float32), \
                patch_pts.reshape(patch_h, patch_w, 2).astype(np.float32), \
                np.array([pts_x/w, pts_y/h, np.deg2rad(theta)]).astype(np.float32)
...
if __name__ == "__main__":
    data = MatchDataset()
    dataloader = DataLoader(data, 16, shuffle = True)
